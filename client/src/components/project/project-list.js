...
export default function ProjectList() {
  ...
  const filters = useSelector((state) => state.projectState.filters);
  const projects = useSelector((state) => state.projectState.projects.filter((item) => {
    return (
      (filters.name.toLowerCase() === '' || item.projectName.toLowerCase().includes(filters.name.toLowerCase())) &&
      ...
      (
        (filters.createdFrom === '' || new Date(filters.createdFrom) <= new Date(item.createdOnTicks)) &&
        (filters.createdTo === '' || new Date(filters.createdTo) >= new Date(item.createdOnTicks))
      ) &&
      (
        (filters.deadlineFrom === '' || new Date(filters.deadlineFrom) <= new Date(item.deadline)) &&
        (filters.deadlineTo === '' || new Date(filters.deadlineTo) >= new Date(item.deadline))
      )
    );
  }));
  ...
  useEffect(() => {
    ...
    if (userInfo.id === 0) {
      ...
      var curSelectedMonthFrom = `${new Date().getMonth() + 1}|${new Date().getFullYear()}`;
      var curSelectedMonthTo = `${new Date().getMonth() + 1}|${new Date().getFullYear()}`;
            ...
            .then(() => dispatch(fetchPriorities())
              ...
              .then(() =>
                dispatch(setProjectFilters({
                  ...
                  selectedMonthFrom: filters.selectedMonthFrom === 0
                    ? new Date()
                    : filters.selectedMonthFrom,
                  selectedMonthTo: filters.selectedMonthTo === 0
                    ? new Date()
        ...
        <div className='card-body'>
          ...
          <div className="form-row">
            ...
            <div className="form-group col-lg-2">
              <label htmlFor="selectedMonthFrom">From Month</label>
              <DatePicker
                ...
                onChange={(date) => handleMonthFromOnChange(date)}
                dateFormat="MM/yyyy"
                showMonthYearPicker
              />
            </div>
            <div className="form-group col-lg-2">
              <label htmlFor="selectedMonthTo">To Month</label>
              <DatePicker
                id="selectedMonthTo"
                ...
                onChange={(date) => handleMonthToOnChange(date)}
                dateFormat="MM/yyyy"
                showMonthYearPicker
              />
            </div>
            ...
            <table className="table table-sm table-hover">
              <thead>
                <tr>
                  ...
                  <th scope="col" colSpan="2">Deadline</th>
                  ...
                  <th >
                    <div className="input-group input-group-sm flex-nowrap">
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => handleFiltersCreatedFromOnChacge(date)}
                        style={{
                  ...
                  <th >
                    <div className="input-group input-group-sm flex-nowrap">
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => handleFiltersCreatedToOnChacge(date)}
                        style={{
                  ...
                  <th >
                    <div className="input-group input-group-sm flex-nowrap">
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => handleFiltersDeadlineFromOnChacge(date)}
                      />
                    </div>
                  </th>
                  <th >
                    <div className="input-group input-group-sm flex-nowrap">
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => handleFiltersDeadlineToOnChacge(date)}
                      />
                    </div>
                  </th>
                  <th>
              ...
              <tbody>
                {projectsTableData.length > 0 &&
                  ...
                  (<tr key={project.id}>
                    ...
                    <td colSpan="2">
                      <i className="bi bi-calendar3"></i> &nbsp;
                      {fixDate(project.createdOnTicks)}
                    </td>
                    <td ><b>{project.projectName}</b></td>
                    <td >{project.description}</td>
                    <td className="text-center">
                      {Number.parseFloat(project.duration).toFixed(2)}{' '}
                    ...
                    <td nowrap="true" align="center">
                      ...
                      <button className="btn btn-sm btn-danger shadow-sm"
                        onClick={() => handleDeleteBtn(project.id)}
                        disabled={role !== 'Administrator'}
                      >
