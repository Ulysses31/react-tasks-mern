...
export default function DepartmentForm() {
  ...
  const error = useSelector(
    (state) => state.departmentState.error
  );
  ...
  const handleOnSubmit = (e) => {
    ...
    if (departmentForm._id === '0') {
      // INSERT
      departmentForm._id = null;
      departmentForm.createdBy = stateLogin.title;
      dispatch(insertDepartment(history, departmentForm));
    } else {
      // UPDATE
      departmentForm.updatedBy = stateLogin.title;
      departmentForm.updatedAt = new Date();
